...
class imdb {
 ...
 private:
  ...
  // all following helper functions are described in imdb.cc
  int bActorSearch(const string& player) const;
  int bMovieSearch(const film& movie) const;
  short* getNumMovies(const string& player, char *actor) const;
  void fillFilmVec(short *numMovies, vector<film>& films, bool toPad) const;
  bool needToPadActF(const string& player) const;
  bool isEvenLen(const string& player) const;
  void createFilm(film& movie, char *movieAddr) const;
  bool needToPadMovF(const film& movie) const;
  void fillActVec(short *numActors, int padNum, vector<string> &players) const;
